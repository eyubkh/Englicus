...
export const LessonFooterNeutral = () => {
	...
	const { challenges, currentChallengeIndex, userInput } =
		useContext(LessonState);
	const { choices, correctIndex, difficulty, type } =
	...
	const handlerIsCorrect = () => {
		...
		if (type === "translate") {
			if (correctIndex.length === userInput.length) {
				isCorrect = userInput.every((value, index) => {
	...
	return (
		<LessonFooterNeutralComponent>
			<TransparentButton handler={handlerIsCorrect}>Skip</TransparentButton>
			<GreenButton onClick={handlerIsCorrect}>Check</GreenButton>
		</LessonFooterNeutralComponent>
	);
};
